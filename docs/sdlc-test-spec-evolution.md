...
**P0 工程搭建**
1. runner 工程：`playwright.config.ts`（baseURL 指向 test 环境，`trace: 'retain-on-failure'`，`screenshot: 'only-on-failure'`）；目录 `specs/<分组>/tc-xx.spec.ts`、`fixtures/`、`.auth/`——✅ 落地为项目侧 runner 工程（一键回归脚本模板见 [skills/sdlc-test/templates/run_regression.sh](../skills/sdlc-test/templates/run_regression.sh)）
