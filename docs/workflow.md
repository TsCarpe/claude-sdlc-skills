...
| 层 | 资产 | 一句话职责 | 标记 | 借鉴来源 |
|---|---|---|---|---|
| **上下文层** | 规范入口文档 | 硬约束 + 正例锚点；个人本地约束（不做推测性修改、依赖 API 来源序） | 🏠（[参考实现](dev-standards-reference/claude-entry.md)） | bp+G+自创 |
| | 细则文档库 | 详细执行层：分主题细则 + 自检清单，按需加载 | 🏠（[参考实现](dev-standards-reference/README.md)） | 自创 |
...
| 机制 | 解决什么问题 | 关键实践 |
|---|---|---|
| 分层上下文工程 | 执行 AI 上下文里没有规范 | 三层分工（入口/细则/速查）+ 任务级清单登记规范与代码锚点、拒绝空清单启动；「注入替代复制」防副本漂移（完整示例见 [dev-standards-reference/](dev-standards-reference/README.md)） |
